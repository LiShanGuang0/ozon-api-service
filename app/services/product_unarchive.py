...
from app.services.product_archive import compact_payload, chunked, item_product_id, item_sku, response_items


class ProductUnarchiveService:
    def __init__(
        self,
        *,
    ...
    async def unarchive_products(self, *, payload: dict[str, Any], credentials: OzonCredentials) -> dict[str, Any]:
        ...
        try:
            ...
            self.events.emit(
                ...
                event_type="unarchive",
                status="success",
                message=f"待还原商品信息查询完成，匹配到 {len(precheck_items)} 个商品",
                ref_type="archive_task",
                ref_id=archive_task_id,
                request_id=request_id,
            ...
            target_ids = [
                ...
                and (item.get("before_is_archived") or item.get("before_is_autoarchived"))
                and item["status"] != "not_found"
            ]
            target_ids = list(dict.fromkeys(target_ids))

            unarchive_payload = {"product_id": target_ids}
            unarchive_data: dict[str, Any] = {"chunks": [], "skipped": True}
            if target_ids:
                unarchive_data = {"chunks": []}
                for product_id_chunk in chunked(target_ids, 100):
                    self.events.emit(
                        client_id=credentials.client_id,
                        event_type="unarchive",
                        status="pending",
                        message=f"开始提交商品还原，数量 {len(product_id_chunk)}",
                        ref_type="archive_task",
                        ref_id=archive_task_id,
                        request_id=request_id,
                    ...
                    chunk_response = await self.forwarder.post(OzonEndpoint.PRODUCT_UNARCHIVE, chunk_payload, credentials, request_id)
                    unarchive_data["chunks"].append({"request": chunk_payload, "response": chunk_response})
                    self.events.emit(
                        client_id=credentials.client_id,
                        event_type="unarchive",
                        status="success",
                        message=f"商品还原提交完成，数量 {len(product_id_chunk)}",
                        ref_type="archive_task",
                        ref_id=archive_task_id,
                        request_id=request_id,
                        payload={"product_id": product_id_chunk},
            ...
            final_items = self._build_final_items(operation_items, target_ids, unarchive_data, confirm_by_product_id, confirm_enabled)
            ...
            for item in final_items:
                db_item = {
                    **item,
                    "operation_response_payload": unarchive_data,
            ...
            self.tasks.finish(
                ...
                request_payload=unarchive_payload,
                response_payload=unarchive_data,
            ...
            self.events.emit(
                ...
                message=f"商品还原处理完成：成功 {counts['success']}，失败 {counts['failed']}，跳过 {counts['skipped']}",
                ref_type="archive_task",
                ref_id=archive_task_id,
                request_id=request_id,
            ...
            return {
                ...
                "unarchive_result": unarchive_data,
    ...
    def _emit_item_event(self, client_id: str, task_id: int, request_id: str, item: dict[str, Any]) -> None:
        ...
        if status == "success":
            ...
            message = item.get("skip_reason") or "商品还原跳过"
        self.events.emit(
            client_id=client_id,
            event_type="unarchive",
            status=event_status,
    ...
    def _build_precheck_items(self, precheck_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in precheck_items:
            ...
            is_autoarchived = bool(item.get("is_autoarchived"))
            is_restorable = is_archived or is_autoarchived
            items.append(
                {
                    ...
                    "before_is_autoarchived": is_autoarchived,
                    "after_is_archived": is_archived,
                    "after_is_autoarchived": is_autoarchived,
                    "status": "pending" if is_restorable else "not_archived",
                    "skip_reason": None if is_restorable else "商品未归档，无需还原",
                    "error_message": None,
    ...
    def _build_final_items(
        ...
        unarchive_ok = self._operation_ok(unarchive_data)
        final_items: list[dict[str, Any]] = []
        for item in operation_items:
            product_id = item.get("product_id")
            if item["status"] in {"not_found", "not_archived"}:
                final_items.append(item)
                continue
            if product_id not in target_id_set:
                item["status"] = "skipped"
                item["skip_reason"] = "缺少 product_id，无法调用还原接口"
                final_items.append(item)
                continue
            if not unarchive_ok:
                item["status"] = "failed"
                item["error_message"] = "Ozon 还原接口未返回成功结果"
                final_items.append(item)
                continue
            confirm_item = confirm_by_product_id.get(product_id)
            if confirm_enabled and not confirm_item:
                item["status"] = "failed"
                item["error_message"] = "还原后确认查询未返回该商品"
                final_items.append(item)
                continue
            if confirm_item:
                ...
                item["after_is_archived"] = False
                item["after_is_autoarchived"] = False

            if not item.get("after_is_archived") and not item.get("after_is_autoarchived"):
                item["status"] = "success"
            else:
                item["status"] = "failed"
                item["error_message"] = "还原后确认商品仍处于归档状态"
            final_items.append(item)
        return final_items

    def _operation_ok(self, operation_data: dict[str, Any]) -> bool:
        chunks = operation_data.get("chunks")
        if not chunks:
            return bool(operation_data.get("skipped"))
        for chunk in chunks:
    ...
    def _counts(self, items: list[dict[str, Any]]) -> dict[str, int]:
        ...
        skipped = sum(1 for item in items if item["status"] in {"skipped", "not_found", "not_archived"})
